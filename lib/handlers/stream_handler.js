...
class StreamHandler {
  ...
  async _getVideoStreams(slug, userApi) {
    try {
      // The videos_manifests endpoint takes the slug directly, so no public
      // API round trip to resolve a numeric ID is needed
      const videoStreams = await userApi.getVideoStreams(slug);

      if (!videoStreams || !Array.isArray(videoStreams)) {
        this.logger.warn('No streams returned from user API', {
          slug,
          streamsCount: videoStreams?.length || 0
        });
        return [];
      }

      // Transform streams from user API format to expected format
      const streams = videoStreams.map(stream => ({
        url: stream.url,
