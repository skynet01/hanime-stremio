...
const ESTIMATED_BITRATES_MBPS = new Map([
  [360, 0.48],
  [480, 0.62],
  [720, 1.35],
  [1080, 2.7]
...
class HtvStreamResolver {
  ...
  constructor(config) {
    ...
    this.handshakeUrl = htv.handshakeUrl ||
      'https://hanime-handshake.skynetsource.com/api/v11/handshake';
    this.relaySecret = htv.relaySecret || null;
    this.metadataUrl = htv.metadataUrl || 'https://www.universal-cdn.com/api/v8/video';
    ...
    this.defaultDurationMs = htv.defaultDurationMs ||
      (config && config.api && config.api.defaultDurationMs) ||
      20 * 60 * 1000;
  ...
  /**
   * Fetch duration from the public video endpoint and estimate per-resolution
   * size from conservative bitrates. The playable HLS URLs are fetched only if
  ...
  async _fetchStreamMetadata(slug, sessionToken) {
    ...
    try {
      ...
      const durationMs = Number(
        response.data && response.data.hentai_video && response.data.hentai_video.duration_in_ms
      );
      if (!Number.isFinite(durationMs) || durationMs <= 0) return byHeight;

      ESTIMATED_BITRATES_MBPS.forEach((bitrateMbps, height) => {
        const durationSeconds = durationMs / 1000;
        byHeight.set(height, {
          filesizeMbs: Math.round((bitrateMbps * durationSeconds / 8) * 10) / 10,
          durationMs,
  ...
  async resolveStreams(slug, sessionToken) {
    ...
    if (this.relaySecret) headers.authorization = `Bearer ${this.relaySecret}`;
    ...
    return normalizedSources.map((source) => {
      ...
      const durationMs = primary.durationMs || fallback.durationMs || this.defaultDurationMs;
      const bitrateMbps = ESTIMATED_BITRATES_MBPS.get(Number(source.height)) || 1.35;
      const derivedFilesizeMbs = Math.round(
        (bitrateMbps * (durationMs / 1000) / 8) * 10
      ) / 10;
      const filesizeMbs = primary.filesizeMbs || fallback.filesizeMbs || derivedFilesizeMbs;
      return {
        ...source,
        filesizeMbs,
        durationMs,
        filesizeEstimated: Boolean(
          primary.filesizeEstimated || fallback.filesizeEstimated ||
          (!primary.filesizeMbs && !fallback.filesizeMbs)
        ),
        durationEstimated: Boolean(
          primary.durationEstimated || fallback.durationEstimated ||
