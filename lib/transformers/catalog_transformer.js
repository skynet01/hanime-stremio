...
function toStremioCatalogItem(catalog) {
  ...
  return {
    ...
    poster: catalog.poster_url || proxyURLById(prefixedId, 'poster'),
    background: catalog.cover_url || undefined,
...
function toStremioSeriesCatalogItem(seriesItem) {
  ...
  const item = {
    ...
    // proxy only as a fallback — see toStremioCatalogItem.
    poster: seriesItem.poster_url || proxyURLById(prefixedId, 'poster'),
    background: seriesItem.cover_url || proxyURLById(prefixedId, 'background'),
