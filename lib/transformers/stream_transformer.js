...
function toStremioStream(stream) {
  ...
  const filesizeMbs = filesizeMissing
    ...
    : suppliedFilesizeMbs;
  const details = [];
  details.push(`💾 ${stream.filesize_estimated || filesizeMissing ? '~' : ''}${filesizeMbs} MB`);
  details.push(`⌚ ${stream.duration_estimated || durationMissing ? '~' : ''}${(durationMs / 60000).toFixed(0)} min`);
