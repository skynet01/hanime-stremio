...
class HanimeUserApi {
  ...
  async login(email, password) {
    ...
    const loginResult = await this.client.reqClient.request(
  ...
  /**
   * Get playable streams for a video by slug
   * Automatically refreshes session token if expired
   *
   * Uses /rapi/v7/videos_manifests/<slug>: the v8 video endpoint's manifest
   * carries a decoy stream (dead streamable.cloud URL, zero duration), while
   * this endpoint returns the real per-resolution CDN URLs and honors the
   * session token for premium entitlements.
   * @param {string} slug - Video slug (e.g., "some-video-1")
  ...
  async getVideoStreams(slug) {
    ...
    const manifest = await this.client.reqClient.request(
      this.client.BASE_URLS.app,
      `/videos_manifests/${encodeURIComponent(slug)}`
    );

    const streams = [];
    const servers = manifest.videosManifest?.servers || [];
  ...
  async _ensureValidSession() {
    ...
    try {
      ...
      this.client = this._createClient(loginResult.sessionToken);
      return true;
    } catch (error) {
