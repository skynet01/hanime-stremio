 ...
 */
async function _cacheWrapUncoalesced(cache, key, method, ttl) {
...
/**
 * Share one cache lookup/fill across concurrent requests for the same key.
 * This prevents cold Stremio request bursts from duplicating upstream work.
 * @private
 */
async function _cacheWrap(cache, key, method, ttl) {
  ...
  const operation = _cacheWrapUncoalesced(cache, key, method, ttl);
  pendingOperations.set(key, operation);

  try {
    return await operation;
  } finally {
    if (pendingOperations.get(key) === operation) {
