...
const config = {
  ...
  api: {
    // Universal CDN serves both public video metadata and the app2-signed
    // search dataset; the client caches that full dataset for local filtering.
    authority: 'www.universal-cdn.com',
    defaultAuthority: 'hw.hanime.tv',
    manifestAuthority: 'cached.freeanimehentai.net',
    searchUrl: 'https://www.universal-cdn.com/api/v10/search_hvs',
    cdnUrl: 'https://hanime-cdn.com',
    // Playable streams come from hanime's /api/v11/handshake flow (guest, no
    // login/CSRF needed). The guest host is reachable from datacenter IPs;
    // *.hanime.tv is Cloudflare-blocked there, but the returned /hls URLs are
    // fetched by the viewer's own device, so hlsHost only needs to resolve for
    // end users. See lib/services/htv_stream_resolver.js.
    htv: {
      handshakeUrl: 'https://guest.freeanimehentai.net/api/v11/handshake',
  ...
  cache: {
    ...
    ttl: {
      ...
      // Streams are handshake-minted hanime.tv/hls URLs whose signed tokens may
      // rotate; keep the cache short so a stale token can't break playback for
      // long (re-minting via the guest handshake is cheap).
      stream: 60 * 60, // 1 hour in seconds
      image: 30 // 30 seconds
    },
  ...
  addon: {
    ...
    logo: process.env.ADDON_LOGO || '/images/logo.png', // Relative path
